...
MODELS=$ROOT/models/NYU/hand_bidirectional_lstm_no_concate

export PYTHONPATH=$ROOT/caffe/python:$ROOT/lib/data/:$ROOT/lib/data_layer:$ROOT/lib/util:$PYTHONPATH

LOG_FILE="$LOG/bidirectional_lstm_no_concate_`date`.txt"
exec &> >(tee -a "$LOG_FILE")
echo Logging to "$LOG_FILE"

$TOOL/caffe train -solver $MODELS/solver_hand_bidirectional_lstm_no_concate.prototxt \
